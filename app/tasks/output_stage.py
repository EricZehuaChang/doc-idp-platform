...
async def _output_stage_inner(file_id: str) -> None:
    ...
    async with sf() as s:
        ...
        if f is None or f.status not in _GENERATE_STATUSES:
            return
        tenant = f.tenant_id
        ...
        if pkg is None or not pkg.output.enabled or pkg.output.action == "off":
            return
        cfg = pkg.output
        revision = f.result_revision or 0
        ...
        existing = (await s.execute(
            select(FileArtifact).where(
                ...
                FileArtifact.config_hash == cfg_hash))).scalars().all()
        if existing and all(a.status == "ready" for a in existing):
            return                      # rerun idempotency (§WP6)

        is_root = f.parent_file_id is None
        children = [] if is_root else []
        if cfg.action == "split":
            children = (await s.execute(
                select(FileRecord)
                .where(FileRecord.parent_file_id == file_id)
                .order_by(FileRecord.created_at, FileRecord.id))).scalars().all()
            if not children:
                return                  # nothing to split (standard file)
            docs = children
        else:
            docs = [f]

        # naming context: rename on a multi-doc file uses the FIRST document's
        # values for {doc_type}/{data.*} (图19); {doc_index} only for split
        original_path = st.local_path(f.storage_path)
        original_ext = Path(f.file_name).suffix
        src_udr = _load_udr(st, f)
        completed_at = f.processed_at or datetime.now(timezone.utc)
        if completed_at.tzinfo is None:
            completed_at = completed_at.replace(tzinfo=timezone.utc)

        plan_rows: list[tuple[FileArtifact, object, dict, dict, bool]] = []
        used: set[str] = set()
        for i, doc in enumerate(docs, start=1):
            meta = (doc.document_meta or {}) if doc is not f else {}
            if doc is f and children:
                first = children[0]
                meta = first.document_meta or {}
            # naming data: split -> the child's own result; rename of a
            # multi-doc file -> the FIRST document's values (图19);
            # rename of a standard file -> the file's own result
            data = (doc.result or {}) if doc is not f else (
                children[0].result if children and children[0].result
                else (f.result or {}))
            clean: dict = {}
            ...
            name, errs = render(pattern, original_name=f.file_name,
            ...
            row = FileArtifact(
                tenant_id=tenant, file_id=file_id, doc_index=doc_index,
        ...
        stale = (await s.execute(
            select(FileArtifact).where(
                ...
                FileArtifact.source_revision < revision))).scalars().all()

    # —— blocking generation outside the session ——
    for row, doc, meta, clean, output_is_pdf in plan_rows:
        try:
            await _generate_one(row, doc, meta, clean, f, docs, cfg,
...
async def _generate_one(row: FileArtifact, doc, meta, clean, f, docs, cfg,
    ...
    searchable = False
    child_udr = _load_udr(st, doc) if doc is not f else src_udr
    ...
    if cfg.action == "rename":
        ...
        doc_path = st.local_path(doc.storage_path) if doc is not f else original_path
        pages = meta.get("source_pages") or []
        if original_ext.lower() == ".ofd":
            raise RuntimeError("unsupported_source_format")
        if original_ext.lower() == ".pdf":
            if doc is not f and doc.storage_path.startswith("split/") \
